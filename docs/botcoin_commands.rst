...
.. include:: _static/includes/botcoin_help.txt
    :code: bash
...
.. include:: _static/includes/botcoin_version.txt
    :code: bash
...
in the ``botcoin`` configuration folder. You can see the location for your
...
.. include:: _static/includes/botcoin_keys_help.txt
    :code: bash
...
Keys generated by ``botcoin`` have a moniker associated with them. The moniker
...
encrypt the keyfile. It writes the encrypted keyfile to the ``botcoin keystore``
...
.. include:: _static/includes/botcoin_help_keys_new.txt
    :code: bash
...
other ``botcoin`` commands.

.. include:: _static/includes/botcoin_help_keys_list.txt
    :code: bash
...
The ``config`` subcommand initialises the configuration for a ``botcoin`` node.
...
- config build - config build creates the configuration for a single-node
                 ...
                 quick and easy way to get started with ``botcoin``.
...
The ``clear`` subcommand safely clears any previous ``botcoin`` configurations.
...
.. include:: _static/includes/botcoin_help_config_location.txt
    :code: bash
...
``botcoin``. It uses one of the accounts from the keystore to define a network
